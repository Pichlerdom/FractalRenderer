...
void Shader::set_sampler(GLuint texture_sampler){
  if(!sampler_location_set){
    sampler_location = glGetUniformLocation(shaderProgram, "sampler");
    sampler_location_set = true;
  }
  
  if(sampler_location != -1){
    glUniform1i(sampler_location, texture_sampler);
  }else{
    printf("warning uniform location not found!");
  }
}
